...
void draw_tris(buffer_t *buf, Triangle far *tris, size_t count)
{
    ...
    for(i = 0; i < count; ++i) {
        ...
        memcpy(tri, tris[i].vertices, 3 * sizeof(Vec3D));
        ...
        for(y = 0; y < total_height; ++y) {
            second_half = y > tri[1].y - tri[0].y || tri[1].y == tri[0].y;
            segment_height = second_half ? tri[2].y - tri[1].y : tri[1].y - tri[0].y;

            alpha = (float)y / total_height;
            beta  = (float)(y - (second_half ? tri[1].y - tri[0].y : 0)) / segment_height;

            a.x = tri[0].x + (tri[2].x - tri[0].x) * alpha;
            a.y = tri[0].y + (tri[2].y - tri[0].y) * alpha;

            b.x = second_half ? tri[1].x + (tri[2].x - tri[1].x) * beta : tri[0].x + (tri[1].x - tri[0].x) * beta;
            b.y = second_half ? tri[1].y + (tri[2].y - tri[1].y) * beta : tri[0].y + (tri[1].y - tri[0].y) * beta;
